...
namespace Discord.Audio
{
    ...
    internal partial class AudioClient : IAudioClient
    {
        ...
        internal AudioClient(SocketGuild guild, int clientId, ulong channelId)
        {
            Guild = guild;
            ChannelId = channelId;
            _audioLogger = DiscordSocketClient.Logger?.CreateLogger($"Audio #{clientId}") ?? NullLogger.Instance;
            ...
            // Fix logger
            _connection = new ConnectionManager(_stateLock, DiscordSocketClient.Logger?.CreateLogger<ConnectionManager>(), 30000,
            ...
            {
                _audioLogger.LogWarning(e.ErrorContext.Error, "Descriptive error");
            ...
            try
            {
                ...
                else if (_connection.State == ConnectionState.Connected)
                {
                    ...
                    else
                    {
                        ...
                        {
                            _audioLogger.LogDebug("Malformed Frame", ex);
                            return;
                        }
