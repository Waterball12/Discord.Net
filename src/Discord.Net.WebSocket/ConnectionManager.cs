...
namespace Discord
{
    internal class ConnectionManager : IDisposable
    {
        ...
        internal ConnectionManager(SemaphoreSlim stateLock, ILogger<ConnectionManager> logger, int connectionTimeout,
                ...
                try
                {
                    ...
                    while (!reconnectCancelToken.IsCancellationRequested)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            {
                                _logger.LogWarning(ex, "A descriptive message");
                                await DisconnectAsync(ex, true).ConfigureAwait(false);
                            }
                            else
                            {
                                _logger.LogError(ex, "A descriptive message");
