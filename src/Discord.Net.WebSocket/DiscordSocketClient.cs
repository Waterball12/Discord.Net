...
namespace Discord.WebSocket
{
    ...
    public partial class DiscordSocketClient : BaseSocketClient, IDiscordClient
    {
        ...
        internal static ILoggerFactory Logger { get; private set; }
...
#pragma warning restore IDISP004
            ...
            : base(config, client)
        {
            ...
            if (Logger == null)
            {
                ...
                Logger.AddProvider(new DefaultLoggerProvider());
            }
            
            _gatewayLogger = Logger.CreateLogger(ShardId == 0 && TotalShards == 1 ? "Gateway" : $"Shard #{ShardId}");

            _connection = new ConnectionManager(_stateLock, Logger.CreateLogger<ConnectionManager>(), config.ConnectionTimeout,
            ...
            {
                _gatewayLogger.LogWarning("Serializer Error", e.ErrorContext.Error);
        ...
        private async Task ProcessMessageAsync(GatewayOpCode opCode, int? seq, string type, object payload)
        {
                    ...
                    case GatewayOpCode.InvalidSession:
                        {
                            ...
                            _gatewayLogger.LogDebug("Failed to resume previous session");
                            ...
                            case "MESSAGE_REACTION_ADD":
                                {
                                    ...
                                    {
                                        SocketUserMessage? cachedMsg = null;
                                        bool isCached = cachedMsg != null;
            ...
            {
                _gatewayLogger.LogError($"Error handling {opCode}{(type != null ? $" ({type})" : "")}", ex);
            }
        }
        ...
        private async Task RunHeartbeatAsync(int intervalMillis, CancellationToken cancelToken)
        {
            ...
            {
                _gatewayLogger.LogError("Heartbeat Errored", ex);
            }
        }
        ...
        private async Task WaitForGuildsAsync(CancellationToken cancelToken, ILogger logger)
        {
            ...
            {
                logger.LogError("GuildDownloader Errored", ex);
            }
        }
        ...
        private async Task TimeoutWrap(string name, Func<Task> action)
        {
            ...
            {
                _gatewayLogger.LogWarning($"A {name} handler has thrown an unhandled exception.", ex);
            }
        }
